...
try {
  ...
  assert.equal(
    persisted["ctx-A"].sessionId,
    "ses_claude_1",
    "persisted map must contain ctx-A → ses_claude_1",
  );
  assert.equal(
    typeof persisted["ctx-A"].createdAt,
    "number",
    "persisted entry must carry createdAt",
  );
  ...
  assert.equal(
    reused.sessionId,
    "ses_claude_1",
    `reuse survived restart: expected ses_claude_1, got ${reused.sessionId}`,
  );
  ...
  assert.equal(
    manager3.getOrCreate("ctx-A").sessionId,
    "ses_claude_1",
    "ctx-A must keep its own session id",
  );
  assert.equal(
    manager3.getOrCreate("ctx-B").sessionId,
    "ses_claude_B",
    "ctx-B must keep its own session id",
  );
  ...
  try {
    ...
    const manager4 = new SessionManager({
      claude: { workingDirectory: staleDir },
      session: { reuseByContext: true, ttl },
    });
    assert.equal(
      manager4.sessions.has("ctx-stale"),
      false,
