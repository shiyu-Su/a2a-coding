...
try {
    ...
    assert.deepEqual(config.claude.settingSources, ["user"], "default must enable the user settings source");
    ...
    assert.deepEqual(config.claude.settingSources, [], "project agentConfig must override settingSources");
    ...
    config = JSON.parse(readFileSync(path, "utf8"));
    assert.equal(config.claude.dangerouslyAllowBypassPermissions, true, "risk config must win over agentConfig");
