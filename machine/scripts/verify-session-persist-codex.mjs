...
function makeStubClient() {
  return {
    ...
    resumeThread(id) {
      resumeCalls++;
      if (!serverThreads.has(id)) throw new Error(`thread not found: ${id}`);
      return { id };
    },
  };
}
...
try {
  ...
  assert.equal(
    persisted["ctx-A"].threadId,
    threadId1,
    "persisted map must contain ctx-A → threadId1",
  );
  assert.equal(
    typeof persisted["ctx-A"].lastAccessedAt,
    "number",
    "persisted entry must carry lastAccessedAt",
  );
  ...
  assert.equal(
    manager2.sessions.get("ctx-A")?.threadId,
    threadId1,
    "restart must restore ctx-A mapping",
  );
  const session2 = manager2.getOrCreate("ctx-A", () => manager2.client.startThread());
  assert.equal(
    session2.threadId,
    threadId1,
    `reuse survived restart: expected ${threadId1}, got ${session2.threadId}`,
  );
  ...
  try {
    ...
    assert.equal(
      manager3.sessions.has("ctx-stale"),
      false,
